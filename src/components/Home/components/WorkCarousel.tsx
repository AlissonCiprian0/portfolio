...
const WorkCarousel = () => {
  ...
  const handleScrollRight = () => {
    if (scrollViewportRef.current) {
      const scrollArea = scrollViewportRef.current?.querySelector(
  ...
  const handleScroll = () => {
    const scrolleableElement = scrollViewportRef.current?.querySelector(
      '[data-radix-scroll-area-viewport]'
    );
  ...
  useEffect(() => {
    ...
    if (scrolleableElement) {
      scrolleableElement.addEventListener('scroll', handleScroll);
    }

    return () => {
      if (scrolleableElement) {
        scrolleableElement.removeEventListener('scroll', handleScroll);
      }
    };
  }, []);
    ...
    <>
      ...
      >
        <div className='pl-0 xl:pl-[22vw]'>
          <div className='flex flex-col items-center space-y-10 p-4 sm:flex-row sm:space-y-0 sm:space-x-4 sm:w-max'>
